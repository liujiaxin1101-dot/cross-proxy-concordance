...
OUTPUT_CSV = os.path.join(os.path.join(PROJECT_ROOT, "data"), "features_raw.csv")
...
print(f"Loaded BW for {len(bw_map)} subjects")
    ...
    for trial_type in TRIAL_TYPES:
        for tn in TRIAL_NUMS:
            ...
            if ic_analog is None:
                print(f"  {fname}: IC not detected (vGRF never > {IC_THRESHOLD_N}N)")
                continue
            ...
            if missing_markers:
                print(f"  {fname}: missing markers {missing_markers}")
                continue
            ...
            print(f"  {trial_type}_t{tn}: hip={hip_flex:.1f}°, valg={knee_valg:.1f}°, "
                  f"trunk={trunk_lean:.1f}°, ankle_angle_sagittal={ankle_angle_sagittal:.1f}°, "
                  f"peakGRF={peak_vgrf_bw:.2f}BW, loadRate={loading_rate:.1f}BW/s")

df = pd.DataFrame(records)
df.to_csv(OUTPUT_CSV, index=False)
print(f"\nSaved {len(df)} records to {OUTPUT_CSV}")
print(f"\nRecords per trial type:")
print(df["trial_type"].value_counts())
print(f"\nSubjects with data: {sorted(df['subject'].unique())}")
